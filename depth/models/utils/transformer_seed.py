...
class seeds_conv(nn.Module):
    def __init__(self, input_dim=100, output_dim=128):
        super(seeds_conv, self).__init__()
        self.conv1 = nn.Conv2d(input_dim, 128, 3, padding=1, stride=1)
        self.conv2 = nn.Conv2d(128, output_dim, 3, padding=1, stride=1)
    ...
    def forward(self, x):
        ...
        x = self.sigmoid(self.conv2(x)) # [b, output_dim, h/4, w/4]
...
class PixelTransformerDecoder(BaseModule):
    ...
    def forward_prediction_heads(self, output, mask_features):
        ...
        outputs_bins = self.bins_embed(bins_queries) # N X 1
        mask_embed = self.mask_embed(bins_queries) # N X C
            
        outputs_mask = torch.einsum("bqc,bchw->bqhw", mask_embed, mask_features)
        outputs_mask = self.hook_identify(outputs_mask)
        # mask_features: W X H X C  这个变量就是F
        ...
        # #################v2##############################################################
        # mask_features_m = mask_features.flatten(2) # 1, C, hw
        # classify = torch.matmul(mask_embed, mask_features_m).permute(0, 2, 1) # bs, hw, n_seeds
        # # 开始对seed_coe进行拆分，分别进行处理；
        # pqr_s = outputs_seeds.split([3, 1], 2)
        # pqr = pqr_s[0]
        # s = pqr_s[1]
        ...
        # coef_feat = torch.cat([pqr_o, s_o], 3).permute(0, 3, 1, 2).contiguous() # bs, 4, h, w
        
        # ################################################

        return outputs_bins, outputs_mask, outputs_class, outputs_seeds

    # query_embed是query， query_feat是pe
    def forward(self,
                ms_feats,
                query_embed,
                query_feat,
        ...
        for idx, layer in enumerate(self.layers):
            ...
            outputs_bins, outputs_mask, outputs_class, outputs_seeds = \
            ...
            predictions_seeds.append(outputs_seeds)

        return predictions_bins, predictions_mask, predictions_class, predictions_seeds
