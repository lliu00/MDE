        ...
        for layer in self.operation_order:
            if layer == 'self_attn':
                ...
                if key.shape[1] != query.shape[1]:
                    key = key.repeat(1,query.shape[1]//key.shape[1],1)
                    value = value.repeat(1,query.shape[1]//value.shape[1],1)
                    key_pos = key_pos.repeat(1,query.shape[1]//key_pos.shape[1],1)
                query = self.attentions[attn_index](
                    query,
                    key,
                    value,
...
class PixelTransformerDecoder(BaseModule):
    ...
    def __init__(self,
        ...
        self.query_partition = nn.Linear(self.embed_dims, self.embed_dims * 4)

    def forward_prediction_heads(self, output, mask_features, cur_level):
        decoder_output = self.decoder_norm(output)
        if cur_level >= 1:
            for _ in range(cur_level):
                decoder_output = decoder_output.reshape(decoder_output.shape[0]*4,decoder_output.shape[1]//4,decoder_output.shape[2])
    ...
    def forward(self,
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if level_index == 2:
                bins_num, batch, feature = output.shape
                # output = output.reshape(bins_num * batch, feature) # view?
                output = self.query_partition(output)
                # output = output.reshape(bins_num, batch * 4, feature)
                output = torch.cat(output.split(feature, dim=2), dim=1)
                query_embed = self.query_partition(query_embed)
                # query_embed = query_embed.reshape(bins_num, batch * 4, feature)
                query_embed = torch.cat(query_embed.split(feature, dim=2), dim=1)
