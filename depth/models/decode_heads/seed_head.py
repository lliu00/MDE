...
def coe_to_depth(max_depth, plane_conf):
    p = plane_conf[:, 0, :, :]
    q = plane_conf[:, 1, :, :]
    r = plane_conf[:, 2, :, :]
    s = plane_conf[:, 3, :, :] # * self.max_depth
    #s = x[:, 3, :, :]
    ...
    batch_size, H, W = plane_conf.size()[0], plane_conf.size()[2], plane_conf.size()[3]
    ...
    disp = (pu + qv + r) * s
    disp = torch.clamp(disp, min=(1 / max_depth)).unsqueeze(1)
...
class BinsFormerDecodeHead(DepthBaseDecodeHead):
    ...
    def forward(self, inputs):
        ...
        if self.dms_decoder:
            ...
            # trans_feats[0].shape = [2, 384, 52, 68]
            # trans_feats[1].shape = [2, 768, 26, 34]
            # trans_feats[1].shape = [2, 1536, 13, 17]
            mlvl_feats = [
                skip_proj(trans_feats[i])
                for i, skip_proj in enumerate(self.skip_proj)
            ]
            # mlvl_feats[0].shape = [2, 512, 52, 68]
            # mlvl_feats[1].shape = [2, 512, 26, 34]
        ...
        predictions_bins, predictions_logits, predictions_class, predictions_seeds = \
             self.transformer_decoder(multi_scale_infos, query_feat, query_pe, per_pixel_feat)

        # NOTE: depth estimation module
        self.norm = 'softmax'

        for item_bin, pred_logit, pred_class, item_seeds in \
            zip(predictions_bins, predictions_logits, predictions_class, predictions_seeds):
            ...
            if self.binsformer is False:
                ...
                # n, dout = centers.size()
                # centers = centers.contiguous().view(n, dout, 1, 1)  # centers.shape = bs X 64 X 1 X 1

                # pred_logit = pred_logit.softmax(dim=1)

                ##########################################################################
                # 这里pred_depth应该修改成由平面系数回归出深度
                # item_seeds Bs X 64 X 4
                # pred_logit Bs X 64 X W X H 
                bs, n, coe_num = item_seeds.size()

                # item_seeds = item_seeds.contiguous().view(bs, n, coe_num, 1)  # item_seeds.shape = bs X 64 X 4 X 1
                # pred_depth = torch.sum(pred_logit * item_seeds, dim=1, keepdim=True)
                ...
                # 调回去 pred_depth.shape = Bs X 1 X W X H
